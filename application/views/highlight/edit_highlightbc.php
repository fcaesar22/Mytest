...
<form method="POST" action="<?php echo base_url("highlight/update_highlight"); ?>" id="form_submit">
    <div class="row" align="center">
        <div class="col-md-12">
            <div class="form-group">
                <label>Category</label>
                <select class="form-control" name="cat_highlight" id="cat_highlight" required>
                    <?php foreach($cattype as $row):?>
                        <option value="<?php echo $row->category_covers;?>"><?php echo $row->cat_highlight;?></option>
                    <?php endforeach;?>
                </select>
            </div>
        </div>
        ...
        <div class="col-md-12">
            <div class="form-group">
                ...
                <select class="form-control" name="type_highlight" id="type_highlight" required>
                    <?php foreach($cattype as $row):?>
                        <option value="<?php echo $row->type_goto;?>"><?php echo $row->type_highlight;?></option>
                    <?php endforeach;?>
                </select>
            </div>
        </div>
...
$(document).ready(function() {
    ...
    function get_poster_highlight(){
        ...
        $.ajax({
            url : "<?php echo site_url('highlight/get_poster_highlight');?>",
            method : "POST",
            ...
            success : function(data){
                    ...
                    $('[name="product_id"]').val(data.covers_id).trigger('change');
                    ...
                    // $('[name="cat_highlight"]').val(data.category_highlight).trigger('change');
                    $('[name="type_highlight"]').val(data.type_highlight).trigger('change');
                    $('[name="content_highlight"]').val(data.id_goto).trigger('change');
...
function getUrlContent(){
    ...
    var images = 'http://wp.dens.tv/img/denslife_v1/1280x720/thumbnail/'+image.replace(/.*\//, '');
...
$(document).ready(function () {
    ...
    $('#uploadcontent').click(function () {
        $.ajax({
            url: '<?php echo base_url() ?>highlight/get_token',
            success: function(data){
    ...
    $('.callfunction2').click(function () {
        ...
        $.ajax({
          type: "GET",
          url: '<?php echo base_url() ?>highlight/compare_image/',
          success: function(data){
