...
<style>
    ...
    .trumbowyg-box,
    .trumbowyg-editor {display: block;position: relative;border: 1px solid #DDD;width: 50%;min-height: 300px;margin: 17px auto;}
    .trumbowyg-box .trumbowyg-editor {margin: 0 auto;}
    .trumbowyg-box.trumbowyg-fullscreen {background: #FEFEFE;}
    .trumbowyg-editor,
    .trumbowyg-textarea {position: relative;-webkit-box-sizing: border-box;-moz-box-sizing: border-box;box-sizing: border-box;padding: 1% 2%;min-height: 300px;width: 100%;border-style: none;resize: none;outline: none;}
</style>
...
<div class="content__inner">
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-md-3">
                    <a href="<?php echo site_url('image/imagedenslife/edit_poster/'.$article_id);?>" class="card-title"><i class="btn btn-light" class="card-title">Back</i></a>
                </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <hr>
                    <br>
                    <form name="form_img" id="form_img" method="POST" action="<?php echo base_url("image/imagedenslife/save_content"); ?>">
                        <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <hr>
                    <br>
                    <form name="form_vid" id="form_vid" method="POST" action="<?php echo base_url("image/imagedenslife/save_video"); ?>">
                        <div class="col-md-6">
                            <table style="width: 100%" class="table">
                                ...
                                <tr id="bar0" class="ris1">
                                    ...
                                    <td class="video">
                                        <div class="col-xl-2 col-lg-3 col-sm-4 col-6" align="center">
                                            <div class="contacts__item" id="vid" align="center">
                                                ...
                                                <a class="btn btn-light btn-sm" name="stream_url[]" onclick="show_modal('video', 'vid_pos', 'vid_001')">Choose Video</a>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td><input type="hidden" class="form-control input-sm" name="stream_type[vid_001][]" value=""></td>
                                    <td><input type="hidden" class="form-control input-sm" name="idprod[vid_001][]" value=""></td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
            ...
            <div class="modal-body">
                <div class="tab-container">
                    <ul class="nav nav-tabs nav-fill" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" data-toggle="tab" href="#inactive_art" role="tab">Embed Videos</a>
                        </li>
                    </ul>
...
<div class="modal fade" id="modal-pic" tabindex="-1" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title pull-left">Select an image</h5>
            ...
            <div class="modal-body">
                <div id="changing-pic">
                    <select class='image-picker show-html' name="pic_sel">
                        <?php
                        if($gallery1!=null){
                            for ($i=0; $i < count($gallery1) ; $i++) {
                                $counter = $i+1;
            ...
            <div class="modal-footer">
                <button id="upload_pic" type="button" class="btn btn-link">Upload Image</button>
                <button type="button" class="btn btn-link" onclick="getUrlPic()">Save</button>
                <button id="back_pic" type="button" class="btn btn-link">Images</button>
                <button type="button" class="btn btn-link" data-dismiss="modal">Close</button>
            </div>
        </div>
    </div>
...
function getUrlVideo(){
    ...
    $('[name="stream_type['+info_id+'][]"]').val(streamtype);
    $('[name="idprod['+info_id+'][]"]').val(prodid);
...
// for gallery video & embed video
function getUrlPic(){
    var image = $('#modal-pic .image_picker_selector').find('.selected').find('img').attr('src');
    // get required element from modal data attribute
    var info_img = $('#modal-pic').attr('data-img-name');
    var info_id = $('#modal-pic').attr('data-id-name');
    // set value into element
    $('#' + info_img).attr('src', image);
    $('#' + info_id).val(image);
    $('#embed_pos').attr('src', image);
    $('#url_image_video').val(image);
...
function getUrlEmbed(){
    $.ajax({
        ...
        success: function (data) {
            console.log(data)
            var editorVal = $('#embed_video').val();
            var newHtml = editorVal + data.html;
            $('#embed_video').trumbowyg('html', newHtml);
            $('#embed_url').val(data.url);
            $('#url_video_final').val(newHtml);
            var infobar = $(".ris1").length;
            var prodid = 'EMB_'+'<?php echo $article_id?>'+'_'+infobar;
            $('#streamtype').val('EMB')
            $('#prodid').val(prodid);
...
$(document).ready(function () {
    ...
    $('#back_img').click(function () {
        ...
        $.ajax({
            ...
            success: function(data){
                console.log(data);
                var _gallery = "<select class='image-picker show-html' id='vid_sel'>";
                data = JSON.parse(data);
                if(data!=null){
                    for (var i = 0; i < data.length; i++) {
                        var _counter = parseInt(i+1);
                        var _gallery = _gallery + "<option data-img-src='"+data[i]+"' data-img-alt='Image "+_counter+"' value='"+data[i]+"'> Image "+_counter+"</option>";
                    }
                }
                var _gallery = _gallery+"</select>";
...
$(document).ready(function () {
    ...
    $('[name=vid_sel]').on('change', function(e){
        ...
        var infobar = $(".ris1").length;
        var prodid = 'AVO_'+'<?php echo $article_id?>'+'_'+infobar;
    ...
    $('#back_pic').click(function () {
        ...
        $.ajax({
            ...
            success: function(data){
                console.log(data);
                var _gallery = "<select class='image-picker show-html' id='pic_sel'>";
                data = JSON.parse(data);
                if(data!=null){
                    for (var i = 0; i < data.length; i++) {
                        var _counter = parseInt(i+1);
                ...
                var _gallery = _gallery+"</select>";
                console.log(_gallery);
                $('#changing-pic').html(_gallery);
                $("select").imagepicker();
            }
        });
    });

    $('#embed_video').trumbowyg({
...
$(document).ready(function (){
    $("body").on('click', '.btn-add-vid', function (e) {
        ...
        '<td><input type="hidden" class="form-control input-sm" name="stream_type[vid_00'+ $no +'][]" value=""></td>' +
        '<td><input type="hidden" class="form-control input-sm" name="idprod[vid_00'+ $no +'][]" value=""></td>' +
        '</tr>';
