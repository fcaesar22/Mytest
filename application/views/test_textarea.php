...
<style>
	...
	.form-control{color: rgba(0, 0, 0, 0.85);border: 1px solid rgb(0, 0, 0);}
	.form-control, .form-control:disabled, .form-control:focus, .form-control[readonly] {background-color: #cac9c9;color: rgba(0, 0, 0, 0.85);}
</style>

<body>
	<form action="<?php echo site_url('test_pdf/save_text');?>" method="post">
		<div class="row">
			<div class="col-md-12">
				<div class="form-group">
                    <textarea class="form-control" rows="5" name="url_test" id="url_test" placeholder="Let us type some lorem ipsum...."></textarea>
                    <i class="form-group__bar"></i>
                    <a class="btn btn-info" onclick="selectText()">Select text</a>
                    <input type="hidden" name="urlstream">
                </div>
			</div>
			<br>
	...
	<script type="text/javascript">
		function selectText() {
			const input = document.getElementById('url_test');  
			input.focus();
		...
		function getUrlVid(){
		    ...
		    $('#urlstream').val(image);
		    if (image != null) {
		    	var genstream = "{image,id}";
		    }
		    var $txt = jQuery("#url_test");
		    var caretPos = $txt[0].selectionStart;
		    var textAreaTxt = $txt.val();
		    $txt.val(textAreaTxt.substring(0, caretPos) + genstream + textAreaTxt.substring(caretPos) );

	      	// $('#url_test').val(video_sel);
		    // hide modal
		    $('#modal-vd').modal('hide');
		}
